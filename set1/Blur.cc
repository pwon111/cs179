...
int large_gauss_test(int argc, char **argv){
    ...
    // Store each channel's data on the GPU here
    float *dev_input_data;
    cudaMalloc((void **) &dev_input_data, N * sizeof(float));

    // Store our impulse response on the GPU here, and copy impulse response
    ...
    // Store our output audio signal on the GPU here
    float *dev_out_data;
    ...
    for (int ch = 0; ch < nChannels; ch++){
        ...
        // Copy the input signal from host memory to the GPU

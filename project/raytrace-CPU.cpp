...
/**
 * Loops through every pixel on the screen and traces a ray from the camera to
 * the superquadric, in order to calculate that pixel's color using the Phong
 * lighting model
 */
void raytrace() {
    // Get our camera-space basis vectors
    Vector3f up(0.0, 1.0, 0.0);
    Vector3f e1, e2, e3;
    e3 = normalize(target - cam_pos);
    e2 = normalize(up - e3 * up.dot(e3));
    // The notes have e2 x e3, but I'm pretty sure this is right since it gives
    // you a right-pointing vector for e1
    e1 = e3.cross(e2);

    // Loop through each pixel position, send a ray from the camera through the
    // pixel on the film plane, and calculate the lighting if it hits the
    // superquadric's surface
    Vector3f av, color;
    Intersection intersect;
    for (int i = 0; i < XRES; i++) {
        for (int j = 0; j < YRES; j++) {
            intersect.t = -1.0;
            // Express a in terms of our basis
            av = Fd * e3 + (i - XRES / 2) * (Fx / XRES) * e1
                 + (j - YRES / 2) * (Fy / YRES) * e2;
            // Trace the ray, and if it hits a surface, calculate the lighting
            intersect = get_nearest_intersection(av, cam_pos, NULL);
            if (intersect.t != -1.0) {
                color = get_lighting(intersect, cam_pos, 0);
                screen[i * YRES + j] = color;
            }
        }
    }
}
...
int main(int argc, char **argv) {
    screen = (Vector3f *) malloc(XRES * YRES * sizeof(Vector3f));
    spheres = (Sphere *) malloc(SPHERE_COUNT * sizeof(Sphere));
    lights = (Point_Light *) malloc(LIGHT_COUNT * sizeof(Point_Light));
    
    Sphere *sphere = spheres;
    sphere->density = 1.0;
    sphere->radius = 0.5;
    sphere->loc = Vector3f(0.0, 0.0, -20.0);
    sphere->vel = Vector3f(0.0, 0.0, 0.0);

    sphere = spheres + 1;
    sphere->density = 1.0;
    sphere->radius = 0.5;
    sphere->loc = Vector3f(-0.9, 0.0, -13.0);
    sphere->vel = Vector3f(0.0, 0.0, 0.0);

    Point_Light *light = lights;
    light->position = Vector3f(0.0, 2.0, -13.0);
    light->color = Vector3f(0.0, 1.0, 0.0);
    light->attenuation_k = 0.0;

    light = lights + 1;
    light->position = Vector3f(2.0, -2.0, -13.0);
    light->color = Vector3f(1.0, 0.0, 0.0);
    light->attenuation_k = 0.0;
    ...
    for (int i = YRES - 1; i >= 0; i--)
        for (int j = 0; j < XRES; j++)
            printf("%d %d %d\n", (int) screen[j * YRES + i][0],
                                 (int) screen[j * YRES + i][1],
                                 (int) screen[j * YRES + i][2]);
}

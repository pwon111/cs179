...
#define STOP_RECORD_TIMER(name) {                           \
    ...
    for (int ch = 0; ch < nChannels; ch++){
        ...
        // Copy this channel's input and impulse response data to the GPU
        cudaMemcpy(dev_input_data, input_data, N * sizeof(cufftComplex),
                   cudaMemcpyHostToDevice);
        cudaMemcpy(dev_impulse_v, impulse_data,
                   impulse_length * sizeof(cufftComplex),
                   cudaMemcpyHostToDevice);

        // Pad the input and impulse data with 0s to the end
        ...
        // Run a forward in-place DFT on the input signal and impulse response
        ...
        // Run an inverse in-place DFT on the output signal
        ...
        // Allocate GPU memory to store the maximum magnitude and set it to 0
        ...
        // Copy the output signal back from the GPU to the host
